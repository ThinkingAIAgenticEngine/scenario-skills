...
name: "Egret"
wikiToken: KRmJwCLouikD7NkD9kucQkjUnDf
parentWikiToken: BAU2w0eipiqGnckeUsIc0mYQnZc
updateTime: 1751966093000
sourceUrl: https://docs-v2.thinkingdata.cn/?version=6.0&lan=zh-CN&code=egret_sdk_installation
---
...
Egret SDK supported platforms: HTML5, iOS, Android, WeChat Mini Game, Baidu Mini Game, Xiaomi Quick Game, OPPO Mini Game, vivo Mini Game, QQ Mini Game, 360 Mini Game, ByteDance Mini Game, Huawei Quick Game, Alipay Mini Game, Taobao Creative Interactive Mini Program, Facebook.
...
This document applies to v3.0.0 and later versions, for historical versions please refer to Egret Integration Guide (V2), SDK Download (v2.2.4)

:::

### I. Integrate SDK

Download and unzip Egret SDK.

Introducing SDK library is the same as introducing other third-party libraries, import the library in egretProperties.json and compile the engine.
...
After importing into the project, compile the engine to use the SDK library.
...
When a user performs login behavior, you can call `login` to set the user's account ID. TE platform prioritizes account ID as identity identification, and the set account ID will be saved. Calling `login` multiple times will override the previous account ID:
...
**Please note, this method will not upload user login event**

#### 1.3 Clear Account ID

After a user performs logout behavior, you can call `logout` to clear the account ID. Before calling `login` again, the visitor ID will be used as the identity identification ID:
...
Please note, this method will not upload user logout event

### II. Send Events

#### 2.1 Ordinary Events

You can directly call `track` to upload custom events. It is recommended that you set event properties and conditions for sending information based on your previously documented tracking plan. Here is an example of purchasing a product:
...
- `track` interface has two parameters, the first parameter is the event name, the second parameter is the event properties
- The event name is a string, can only start with a letter, can contain numbers, letters and underscores "\_", maximum length 50 characters, case-insensitive.
- Event properties are a JS object, each element represents a property.
- The name of the element corresponds to the property name, can only start with a letter, contain numbers, letters and underscores "\_", maximum length 50 characters, case-insensitive.
- The Value of the element is the property value, supports `String`, `Number`, `Boolean`, `Date`, `Object` and `Array`; Content in `Object` can be `String`, `Number`, `Boolean`, `Date`, `Array` (content as string); Content in `Array` can be `Object` and `String`

#### 2.2 First-time Event

First-time events refer to events that will only be recorded once for a certain device or other dimensional ID. For example, in some scenarios, you may want to record the first occurrence of an event on a certain device, then you can use first-time events to report the data.
...
If you want to determine whether it is the first time based on other dimensions other than the device, you can set the first_check_id for the first-time event. For example, if you need to record the first event of a certain account, you can set the account ID as the first_check_id of the first-time event:
...
Event public properties refer to static public properties, only constants can be passed when setting, suitable for setting stable properties. You can call `setSuperProperties` to set public event properties, the format requirements for public event properties are the same as event properties.

<!-- unsupported block: 34 -->

According to property priority, custom properties have higher priority than event public properties, so event public properties can also be used as a default value for a property, set the same Key to override the default value in events that need modification.
...
If you call `setSuperProperties` multiple times to set public event properties, subsequent calls with the same name field will override previous ones, different name fields will be added.

If you need to delete a public event property, you can call `unsetSuperProperty()` to clear one public event property; if you want to clear all public event properties, you can call `clearSuperProperties()`; if you want to get all public event properties, you can call `getSuperProperties`;
...
Dynamic public properties will execute a function during event reporting, and use the return value as the value of this dynamic public property, adding to the event. You can call the `setDynamicSuperProperties` interface to set dynamic public properties. This interface accepts a function as a parameter.

```
// Set UTC time as event property upload through dynamic public properties
...
The function must return a JS object, each element represents a property. Property format requirements are the same as event properties.

#### 2.6 Record Event Duration

You can call `timeEvent` to start timing, configure the event name you want to time. When you upload that event, a `#duration` property will automatically be added to your event properties to represent the recorded duration in seconds.
...
If you need to upload user properties that should only be set once, you can call `userSetOnce` to set them. When the property already has a value, this information will be ignored.
...
When you need to upload numeric properties, you can call `userAdd` to perform an accumulation operation on the property. If the property has not been set, it will be assigned 0 before calculation
...
The passed value of userUnset is the Key of the property to be cleared.
...
You can call `userAppend` to append elements to Array (List) type user data.
...
Note: This feature needs to be used with TE platform version 2.5 and later

#### 3.7 userUniqAppend

Starting from v2.1.0, you can call `userUniqAppend` to **deduplicate** and append elements to Array (List) type user data.
...
Note: This feature needs to be used with TE platform version 3.6 and later

### V. Other Features

#### 5.1 Get Device ID

You can call `getDeviceId()` to get the device ID, **Due to runtime environment, device ID will be saved in local cache, once user deletes cache, device ID will change, so device ID cannot guarantee stability**
...
For track, userSet, userSetOnce, userAdd, userDel interfaces, onComplete callback can be passed.

onComplete can be passed directly after the original parameter list, or using parameter object method. If using parameter object, the parameter object must include onComplete, otherwise a parameter error will occur.

Using uploading event as an example:
...
**Build iOS platform application, configure iOS project**
...
- Register Native proxy
- Register Native proxy in `AppDelegate.mm`'s `application:didFinishLaunchingWithOptions:` method, call code `[EgretProxyApi registExternalInterface:_native]`

### II. Android Native Support

**Build Android platform application, configure Android project**

- Add Android project dependency files
- `EgretProxyApi.java`
- Add the following configuration dependency in `Project` level `build.gradle` file
...
- Register Native proxy
- Register Native proxy in `MainActivity`'s `onCreate` method, call code `EgretProxyApi.registExternalInterface(nativeAndroid,this)`
...
var config = {
  ...
  autoTrack: {
    ...
    appView: true, // Auto-track page view event (only native effective)
...
var config = {
  ...
  enableLog:true//Enable log switch, default is false
};
TDAnalytics.init(config);
```
...
- `#start_reason`, app start source, parameters from `getLaunchOptionsSync` interface
